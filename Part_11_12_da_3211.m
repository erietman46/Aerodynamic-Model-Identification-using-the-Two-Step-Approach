...
%CREATING THE TRUE STATE VECTOR, WHICH CAN THEN BE COMPARED WITH THE KALMAN
%FILTER OUTPUT
u = vtas .* cos(alpha) .* cos(beta);
v = vtas .* sin(beta);
w = vtas .* sin(alpha) .* cos(beta);

lambda_true = [lambda_Ax; lambda_Ay; lambda_Az; lambda_p; lambda_q; lambda_r];
wind_true   = [Wx; Wy; Wz];

x_true = [xE, yE, zE, ...
    u, v, w, ...
    phi, theta, psi, ...
    lambda_Ax*ones(size(t)), lambda_Ay*ones(size(t)), lambda_Az*ones(size(t)), ...
    lambda_p*ones(size(t)), lambda_q*ones(size(t)), lambda_r*ones(size(t)), ...
    Wx*ones(size(t)), Wy*ones(size(t)), Wz*ones(size(t))];

%CREATING THE NOISE COVARIANCE MATRICES. Q = PROCESS NOISE COVARIANCE
%MATRIX = COVARIANCE OF THE INPUTS. R = MEASUREMENT NOISE COVARIANCE MATRIX
%=COVARIANCE OF THE MEASUREMENTS.
Q = diag([sigma_Ax, sigma_Ay, sigma_Az, sigma_p, sigma_q, sigma_r].^2);

R = diag([sigma_pos, sigma_pos, sigma_pos, ...
    sigma_vel, sigma_vel, sigma_vel, ...
    sigma_att, sigma_att, sigma_att, ...
    sigma_V, sigma_alpha, sigma_beta].^2);

save('part1_da3211_1_preprocessed.mat', ...
    't', 'x_true', 'u_IMU', 'z_m', ...
    'Q', 'R', 'lambda_true', 'wind_true', ...
    'xE', 'yE', 'zE', ...
    'Ax_m', 'Ay_m', 'Az_m', 'p_m', 'q_m', 'r_m', ...
    'x_GPS', 'y_GPS', 'z_GPS', ...
    'u_GPS', 'v_GPS', 'w_GPS', ...
    'phi_GPS', 'theta_GPS', 'psi_GPS', ...
    'V_m', 'alpha_m', 'beta_m');
